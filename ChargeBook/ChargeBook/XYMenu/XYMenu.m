...
static const CGFloat XYMenuWidth = 100; // Menu宽度
static const CGFloat XYMenuItemHeight = 40; // item高度
...
+ (void)showMenuWithTitles:(NSArray *)titles inView:(UIView *)view menuType:(XYMenuType)menuType withItemClickIndex:(ItemClickIndexBlock)block
{
    ...
    [xy_menu showMenuWithTitles:titles inView:view menuType:menuType withItemClickIndex:block];
}
...
{
    XYMenu *xy_menu = [[XYMenu alloc] init];
    [xy_menu showMenuWithTitles:titles menuType:menuType currentNavVC:currentNavVC withItemClickIndex:block];
}
...
- (void)showMenuWithTitles:(NSArray *)titles menuType:(XYMenuType)menuType currentNavVC:(UINavigationController *)currentNavVC  withItemClickIndex:(ItemClickIndexBlock)block
{
    ...
    [self showAnimateMenuWithTitles:titles menuType:menuType withBlock:block];
...
- (void)showAnimateMenuWithTitles:(NSArray *)titles menuType:(XYMenuType)menuType withBlock:(ItemClickIndexBlock)block
{
    __weak typeof(self) weakSelf = self;
    [self.menuView setTitles:titles withRect:self.menuResultRect withMenuType:menuType isDown:_isDown withItemClickBlock:^(NSInteger index) {
